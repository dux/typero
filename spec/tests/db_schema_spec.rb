...
describe Typero do
  ...
  describe 'DB schema access' do
    it 'gets valid schema' do
      ...
      expect(schema[0]).to eq([:string, :name, { limit: 255}])
      ...
      expect(schema[6]).to eq([:string, :eyes, { default: 'blue', limit: 255, null: false}])
      expect(schema[7]).to eq([:integer, :age, { null: false}])
